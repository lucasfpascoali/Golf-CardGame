...
class PlayerInterface:
    ...
    def _init_login_frame(self):
        ...
        logo_label = tk.Label(self._login_frame, image=self._logo_img, bg=self._primary_color)
    ...
    def _create_side_frame(self):
        self._side_frame = tk.Frame(self._game_frame, height=1024, width=295, bg=self._side_color)
    ...
    def _create_player_icons(self):
        ...
        self._player_icon = ImageTk.PhotoImage(img)
        
        # Remote players
        player_icon_label_1 = tk.Label(self._game_frame, image=self._player_icon, bg=self._bg_color)
        player_icon_label_1.place(x=323, y=70)

        player_icon_label_2 = tk.Label(self._game_frame, image=self._player_icon, bg=self._bg_color)
    ...
    def _create_discard_pile(self):
        ...
        self._discard_pile = ImageTk.PhotoImage(img)
        discard_pile_btn = tk.Button(
            self._game_frame,
            image=self._discard_pile,
            bd=0,
            bg=self._bg_color,
            activebackground=self._bg_color,
            highlightthickness=0,
        ...
        self._trash_can = ImageTk.PhotoImage(img)
        trash_can_label = tk.Label(self._game_frame, image=self._trash_can, bg=self._bg_color)
    ...
    def _clear_placeholder(self):
        if self._nickname_entry.get() == "Nickname":
            self._nickname_entry.delete(0, tk.END)
    ...
    def _add_back_cards(self, parent_frame, card_size=(125, 196), clickable=True):
        ...
        for row in range(2):
            for col in range(3):
                ...
                card_btn.grid(row=row, column=col, padx=10, pady=5)
                
                # Add click event only if cards are clickable
                if clickable:
                    card_index = row * 3 + col  # Calculate index based on position
                    card_btn.bind("<Button-1>", lambda _, idx=card_index: self._on_card_click(idx))
