...
class Board:
    def __init__(self, player: Player, initial_cards: list[Card]):
        ...
        self._build_initial_matrix(initial_cards)
        self._reveal_two_random_cards()
    ...
    def _build_initial_matrix(self, cards: list[Card]) -> None:
        if len(cards) != 6:
    ...
    def _reveal_two_random_cards(self):
